...
class InterfazAerolinea:
    ...
    def crear_formulario(self):
        ...
        # ⬇️ estos iban a frame_datos; deben ir dentro de frame_asiento
        self.crear_input(frame_asiento, "Clase (0: Business, 1: Turista)", 1)
        self.crear_input(frame_asiento, "Fila", 2)
        self.crear_input(frame_asiento, "Letra asiento (A-D)", 1)
    ...
    def reservar(self):
        try:
            ...
            mensaje = (
                f"✅ Reserva exitosa:\n"
                f"{nombre} {apellido}, edad {edad}, pasaporte {pasaporte}, "
                f"asiento {fila}{letra}, clase {clase_obj.name}, vuelo {vuelo}\n"
            )
    ...
    def mostrar_ingresos(self):
        ...
        ingresos = 0
        for clase, precio in [
            (Clase.BUSINESS, self.sistema.PRECIO_BILLETE_BUSINESS),
            (Clase.TURISTA, self.sistema.PRECIO_BILLETE_TURISTA)
        ]:
            for fila in range(1, avion.get_numero_filas(clase) + 1):
                for b in range(1, avion.get_butacas_por_fila() + 1):
                    p = avion.get_pasajero(fila, b, clase)
    ...
    # ----------------- HELPERS PARA PDF -----------------
    def _pasajeros_de_vuelo(self, vuelo):
        avion = vuelo.avion
        filas = []
        for clase in [Clase.BUSINESS, Clase.TURISTA]:
            for f in range(1, avion.get_numero_filas(clase) + 1):
                for b in range(1, avion.get_butacas_por_fila() + 1):
                    p = avion.get_pasajero(f, b, clase)
                    if p:
                        letra = chr(ord("A") + b - 1)
                        filas.append({
                            "nombre": p.nombre,
    ...
    def exportar_pdf_pasajeros(self):
        try:
            vuelo = self.obtener_vuelo_actual()
            ...
            titulo = f"Manifiesto de Pasajeros • Vuelo {vuelo.origen} → {vuelo.destino}"
            subtitulo = f"Fecha: {vuelo.fecha} • Modelo: {vuelo.avion.modelo} • Generado: {datetime.now().strftime('%d/%m/%Y %H:%M')}"
            elems.append(Paragraph(titulo, styles["Title"]))
            elems.append(Paragraph(subtitulo, styles["Normal"]))
            elems.append(Spacer(1, 12))

            headers = ["#", "Nombre", "Edad", "Pasaporte", "Teléfono", "Asiento", "Clase"]
            table_data = [headers]
            for i, r in enumerate(datos, start=1):
                table_data.append([i, r["nombre"], r["edad"], r["pasaporte"], r["telefono"], r["asiento"], r["clase"]])
            ...
            tabla.setStyle(TableStyle([
                ...
                ("ROWBACKGROUNDS", (0,1), (-1,-1), [colors.whitesmoke, colors.azure]),
            ]))
            elems.append(tabla)
            elems.append(Spacer(1, 12))
            ...
            for clase, precio in [
                ...
                for f in range(1, avion.get_numero_filas(clase) + 1):
                    for b in range(1, avion.get_butacas_por_fila() + 1):
                        p = avion.get_pasajero(f, b, clase)
                        if p:
                            desc = self.sistema.DESCUENTO_INFANTIL if p.edad < 15 else 0
                            ingresos += precio * (1 - desc/100)

            elems.append(Paragraph(f"Total pasajeros: <b>{len(datos)}</b>", styles["Normal"]))
            elems.append(Paragraph(f"Ingresos estimados: <b>${ingresos:.2f}</b>", styles["Normal"]))
